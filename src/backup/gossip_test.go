...
var _ = Describe("Gossip", func() {
	...
	Describe("R=1, W=1, ClusterSize=2", func() {
		BeforeEach(func() {
	...
	Describe("R=1, W=1, ClusterSize=3", func() {
		BeforeEach(func() {
	...
	Describe("R=2, W=2, ClusterSize=3", func() {
		BeforeEach(func() {
			// StartingPort: 8000, R-Value: 2, W-Value: 2, ClusterSize: 3
			sc = NewServerCoordinator(8000+config.GinkgoConfig.ParallelNode*100, 2, 2, 3)
		})
		...
		Context("when there are conflict entries in remote server", func() {
			It("should replicate entries and keep conflict remote entries.", func() {
				sc.GetClient(0).Put(MakePutFreshEntry("k0", []byte("v0-0")))
				sc.GetClient(2).Put(MakePutFreshEntry("k0", []byte("v0-1")))
			...
			It("should remove remote descendant entries", func() {
				...
				sc.GetClient(2).Put(MakePutFromVectorClockMapAndValue(
					"k0",
					map[string]uint64{sc.GetID(0): 1},
					[]byte("v0-2"),
				))
